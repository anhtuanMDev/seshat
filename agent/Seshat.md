...
> 6. **Accidental Deletion of Other Books (Partial Load Sync Guard):** If a user deep-links directly to a specific book, the client only loads that single book into `appStore.books` (the list starts empty). To prevent a subsequent sync from deleting all other books on GitHub (since sync rebuilds the Git tree without `base_tree`), the client tracks if the full book index has been loaded via the persistent `appStore.isBookListLoaded` flag. The backend uses this flag to automatically preserve all other books in the repository if the client sync is partial.
