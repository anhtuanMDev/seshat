...
function scoreFighter(char: any, events: any[], atEventId?: string) {
  let score = 0;
  const notes: any[] = [];

  const resolveEvent = atEventId ? events.find((e: any) => e.id === atEventId) : [...events].sort((a, b) => b.time - a.time).find((e: any) => (e.characters || []).includes(char.id));
  ...
  const cursedEquipped = equippedItems.filter((eq: any) => eq.curses && eq.curses.trim());
  ...
  const ARC_MOD: Record<string, number> = { Unaware: 0, Questioning: 0.2, Resisting: 0.5, Breaking: 1, Transforming: 1.5, Integrated: 2 };
...
export default function FightPage() {
  ...
  const charA = chars.find((c: any) => c.id === aId);
  const charB = chars.find((c: any) => c.id === bId);
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 1fr", gap: "0 24px", alignItems: "center", marginBottom: 28 }}>
            <div>
              ...
              {charA && (
                <div style={{ marginTop: 8 }}>
                  <EventPicker label="At timeline point" value={aEventId} onChange={setAEventId} events={events.filter((e: any) => (e.characters || []).includes(charA.id))} />
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {charB && (
                <div style={{ marginTop: 8 }}>
                  <EventPicker label="At timeline point" value={bEventId} onChange={setBEventId} events={events.filter((e: any) => (e.characters || []).includes(charB.id))} />
                </div>
              )}
            </div>
          </div>
            ...
            <>
              ...
              <div style={S.grid2}>
                <div>
                  ...
                  {resultA!.notes.map((n, i) => <NoteRow key={i} n={n} />)}
                ...
                <div>
                  ...
                  {resultB!.notes.map((n, i) => <NoteRow key={i} n={n} />)}
                  {!resultB!.notes.length && <p style={S.dim}>No data found.</p>}
                </div>
              </div>

              <hr style={{ border: "none", borderTop: "1px solid #e0ddd8", margin: "24px 0 12px" }} />
