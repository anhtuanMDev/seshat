...
export default function WorldPage() {
  ...
  const { register, control, reset, setValue, getValues, formState: { isDirty } } = useForm<WorldForm>({
    defaultValues: {
  ...
  return (
    <div ref={ref}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, gap: 16 }}>
        ...
        <button disabled={!isDirty || isSaving} onClick={() => onSubmit(getValues())} title="Save changes" style={{ ...S.ghost, fontSize: 11, letterSpacing: 1, color: "var(--color-green)", flexShrink: 0, display: "flex", alignItems: "center", gap: 3, opacity: (!isDirty || isSaving) ? 0.5 : 1, cursor: (!isDirty || isSaving) ? "default" : "pointer" }}><SaveIcon sx={{ fontSize: 12 }} />{isSaving ? "saving..." : "save"}</button>
      </div>
      ...
      <Field label="Setting" name="setting" control={control} multi rows={2} placeholder="Time period, place, atmosphere…" />
      <Field label="Themes" name="themes" control={control} multi rows={2} placeholder="The ideas the story is really about…" />
