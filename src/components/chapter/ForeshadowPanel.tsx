...
export function ForeshadowPanel({ foreshadows, chapters, currentChapterId, onAddForeshadow, onUpdateForeshadow, onDeleteForeshadow }: Props) {
  ...
  const renderForeshadow = (f: Foreshadow) => {
    return (
      <EntryBlock key={f.id} onDelete={() => onDeleteForeshadow(f.id)}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              <Sel 
                ...
                opts={[{ label: "None", value: "" }, ...chapters.map(c => ({ label: c.title || c.number, value: c.id }))]}
              />
            </div>
            <div style={{ flex: 1 }}>
              <Sel 
            ...
            <div style={{ flex: 1 }}>
              <Sel 
                ...
                opts={[{ label: "Planted", value: "Planted" }, { label: "Payoffed", value: "Payoffed" }, { label: "Abandoned", value: "Abandoned" }]}
              />
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {relevantForeshadows.map(renderForeshadow)}
      </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12 }}>
          ...
          {showAll && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
              {otherForeshadows.map(renderForeshadow)}
            </div>
          )}
        </div>
      )}
