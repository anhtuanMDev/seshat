export function buildExport(state: any): string {
  ...
  const evLabel = (id: string) => { const e = ev(id); return e ? `[T${e.time}] ${e.title}` : ""; };
  ...
  for (const c of state.characters) {
    ...
    const activeConditions = (c.conditions||[]).filter((cd: any)=>cd.isActive);
    ...
    if (latestEvent || activeConditions.length) {
      ...
      if (activeConditions.length) {
        L.push(`    Active conditions: ${activeConditions.map((cd: any)=>`${cd.name} (${cd.type})`).join(", ")}`);
      }
    }
