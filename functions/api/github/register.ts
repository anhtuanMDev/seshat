...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    const { username, accessCode } = await context.request.json() as { username?: string, accessCode?: string };
    const { GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO } = context.env;

    if (!GITHUB_TOKEN || !GITHUB_OWNER || !GITHUB_REPO) {
      return new Response(JSON.stringify({ error: "Missing GitHub environment variables." }), { status: 500 }) as unknown as CloudflareResponse;
    }
    ...
    if (getRes.ok) {
      ...
      return new Response(JSON.stringify({ error: "Failed to access user database." }), { status: 500 }) as unknown as CloudflareResponse;
    }

    // 2. Check if user already exists
    if (usersData[cleanUsername]) {
      return new Response(JSON.stringify({ error: "Username already exists. Please login instead." }), { status: 409 }) as unknown as CloudflareResponse;
    }
    ...
    if (!putRes.ok) {
      ...
      return new Response(JSON.stringify({ error: "Failed to save registration: " + errText }), { status: 500 }) as unknown as CloudflareResponse;
    }
